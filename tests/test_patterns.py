...
print("\n=== E2. the small-screen layout ===")
doc = open(os.path.join(ROOT_DIR, "templates", "report_doc.html")).read()
chk("there is a small-screen breakpoint at all",
    "@media screen and (max-width: 820px)" in doc)
chk("the sheet stops being a fixed A4 on a phone",
    "height:auto" in doc.split("max-width: 820px")[1][:2600])
chk("and stops being scaled down to fit",
    "zoom:1 !important" in doc.split("max-width: 820px")[1][:2600])
chk("body type comes up to a readable size",
    "font-size:13.5px" in doc.split("max-width: 820px")[1][:2600])
chk("detail rows carry the labels the stacked view prints",
    all(f'data-label="{lbl}"' in doc
        for lbl in ("Zone", "Acres", "What we saw", "Likely cause", "Recommendation")))
chk("the stacked view reads those labels back",
    "content:attr(data-label)" in doc)
chk("print is never reflowed — it is a separate media context",
    "@media print" in doc and "zoom:1;" in doc.split("@media print")[1][:400])
chk("the script leaves the scale alone below the breakpoint",
    "window.innerWidth <= MOBILE" in doc)

print("\n=== E3. the V1 house style is intact ===")
for token, why in [("#030920", "the ink the rest of the product uses"),
                   ("#D6DBD5", "the silver hairline"),
                   ("#6C6C6C", "the dove grey for labels"),
                   ("#F2F5EF", "the canvas"),
                   ("2px solid #030920", "the masthead rule"),
                   ("montserrat-latin-800-normal.woff2", "the bundled face")]:
    chk(f"still uses {why}", token in doc, token)
chk("no score dial survives", "OUT OF 100" not in doc and "gauge" not in doc)
chk("and no ranked list of orders", "Work through these" not in doc)
